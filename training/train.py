...
def split_data(data_df):
    ...
    features = data_df.drop(['Defaulted', 'Index'], axis=1)
    print(f"features: {features}")
    labels = np.array(data_df['Defaulted'])
    print(f"labels: {labels}")
    features_train, features_valid, labels_train, labels_valid = \
        train_test_split(features, labels, test_size=0.2,
                         random_state=0)
    print(f"features_train: {features_train}")
    print(f"features_valid: {features_valid}")
